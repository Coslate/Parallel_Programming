...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    //Allocate pinned and mapped mem in host
    cudaHostAlloc(&values, tpoints*sizeof(float), cudaHostAllocMapped);

    //Invoke retrieving device pointer for mapped mem
    cudaHostGetDevicePointer(&values_d, values, 0 );
    ...
    cuda_update<<<blocks_num, threads_per_block, 0, stream1>>>(values_d, tpoints, nsteps);
