    ...
    */
    ...
    while (!balance) {
        ...
        if(my_rank_rows[my_rank] > 0){
            time0 = MPI_Wtime();
            memcpy(lower, &temp[(my_rank_end[my_rank] - num_pad + 1)][0], N*num_pad*sizeof(int));
            memcpy(upper, &temp[my_rank_start[my_rank]][0], N*num_pad*sizeof(int));
            time1 = MPI_Wtime();
