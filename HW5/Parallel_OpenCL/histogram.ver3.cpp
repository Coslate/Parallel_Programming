    ...
    */
    ...
    if (argc >= 2)
    {
        ...
        for (int i = 0; i < many_img; i++)
        {
            ...
            cl_mem ret_img_d  = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(uint8_t) * 4 * 65536, NULL, &ret_code);
            HANDLE_ERROR(ret_code);
            ...
            */
            ...
            clSetKernelArg(kernel_ret_obj, 2, sizeof(uint32_t), &max);
            ...
            //-------------------Read the result back to host--------//
            HANDLE_ERROR(clEnqueueReadBuffer(command_queue, ret_img_d, CL_TRUE, 0, sizeof(uint8_t) * 4 * 65536, ret->data, 0, NULL, NULL));
