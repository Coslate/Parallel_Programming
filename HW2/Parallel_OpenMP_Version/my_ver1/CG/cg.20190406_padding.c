...
static double z[NA+2][DOUBLE_PAD_SIZE] = {0};
static double p[NA+2][DOUBLE_PAD_SIZE] = {0};
static double q[NA+2][DOUBLE_PAD_SIZE] = {0};
static double r[NA+2][DOUBLE_PAD_SIZE] = {0};
...
#pragma omp parallel for default(shared) private(j) reduction(+:norm_temp1,norm_temp2)    
    for (j = 0; j < lastcol - firstcol + 1; j++) {
      norm_temp1 = norm_temp1 + x[j] * z[j][0];
